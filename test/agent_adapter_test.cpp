...
protected:
  void SetUp() override
  {
    ...
    m_device =
        parser->parseFile(PROJECT_ROOT_DIR "/test/resources/samples/test_config.xml", printer.get()).front();
  ...
  void createAgent(ConfigOptions options = {}, std::string deviceFile = "/test/resources/samples/test_config.xml")
  {
    m_agentTestHelper->createAgent(deviceFile, 8, 4, "2.0", 25, false, true,
                                   options);
...
TEST_F(AgentAdapterTest, should_create_device_when_option_supplied)
{
  ...
  auto port = m_agentTestHelper->m_restService->getServer()->getPort();
  auto adapter = createAdapter(port, {{configuration::UpdateDeviceXml, true}});
