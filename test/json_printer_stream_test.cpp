...
protected:
  void SetUp() override
  {
    ...
    m_devices =
        m_config->parseFile(PROJECT_ROOT_DIR "/test/resources/samples/SimpleDevlce.xml", m_xmlPrinter.get());
  }
...
{
  m_printer = std::make_unique<printer::JsonPrinter>(2, true);
  m_devices = m_config->parseFile(PROJECT_ROOT_DIR "/test/resources/samples/min_config2.xml", m_xmlPrinter.get());
