...
protected:
  ...
  void createServer(const ConfigOptions& options) {
    ...
    MergeOptions(opts, {{ServerIp, "127.0.0.1"s},
                        ...
                        {AutoAvailable, false},
                        {RealTime, false}});
    m_server = std::make_shared<mtconnect::mqtt_server::MqttTcpServer>(m_agentTestHelper->m_ioContext, opts);
  }
...
TEST_F(MqttEntitySinkTest, mqtt_entity_sink_should_support_authentication)
{
  ...
  handler->m_connected = [&connected](std::shared_ptr<MqttClient>) {
    connected = true;
  };
...
TEST_F(MqttEntitySinkTest, mqtt_entity_sink_should_support_qos_levels)
{
  ...
  auto handler = std::make_unique<ClientHandler>();
  bool received = false;
  handler->m_receive = [&received](std::shared_ptr<MqttClient> client, const std::string& topic, const std::string& payload) {
    received = true;
  };
...
TEST_F(MqttEntitySinkTest, mqtt_entity_sink_should_support_retained_messages)
{
  ...
  handler->m_receive = [&retainedReceived, &retainedPayload](std::shared_ptr<MqttClient> client, const std::string& topic, const std::string& payload) {
...
TEST_F(MqttEntitySinkTest, mqtt_entity_sink_should_publish_last_will)
{
  ...
  handler->m_receive = [&lastWillReceived](std::shared_ptr<MqttClient> client, const std::string& topic, const std::string& payload) {
    if (topic.find("Availability") != std::string::npos) {
