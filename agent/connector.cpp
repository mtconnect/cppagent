...
void Connector::connect()
{
  ...
  try
  {
    ...
    if (status < 0)
    {
      sLogger << LWARN << "(Port:" << m_localPort << ")" << "connect: Could not write initial heartbeat: " << intToString(status);
      close();
      return;
    }
    ...
    while (m_connected)
    {
      ...
      {
        sLogger << LDEBUG << "(Port:" << m_localPort << ")" << "Connection was closed, exiting adapter connect";
        break;
      }
      
      if (!m_connected) {
        sLogger << LDEBUG << "(Port:" << m_localPort << ")" << "Connection was closed during read, exiting adapter";
        break;
      }
      ...
      else if (status == TIMEOUT && !m_heartbeats && ((int) (stamper.get_timestamp() - now) / 1000) >= timeout) 
      {
        ...
        sLogger << LERROR << "(Port:" << m_localPort << ")" << "connect: Did not receive data for over: " << (timeout / 1000) << " seconds";
        break;
      }
      ...
      if (m_heartbeats)
      {
        ...
        {
          sLogger << LERROR << "(Port:" << m_localPort << ")" << "connect: Did not receive heartbeat for over: " << (m_heartbeatFrequency * 2);
          break;
        }
        else if ((now - m_lastSent) >= (uint64) (m_heartbeatFrequency * 1000)) 
        {
          ...
          sLogger << LDEBUG << "(Port:" << m_localPort << ")" << "Sending a PING for " << m_server << " on port " << m_port;
          status = m_connection->write(ping, strlen(ping));
          if (status <= 0)
          {
            sLogger << LERROR << "(Port:" << m_localPort << ")" << "connect: Could not write heartbeat: " << status;
            break;
          }
          m_lastSent = now;
        }
    ...
    sLogger << LERROR << "(Port:" << m_localPort << ")" << "connect: Connection exited with status: " << status;
  ...
  {
    sLogger << LERROR << "(Port:" << m_localPort << ")" << "connect: Exception in connect: " << e.what();
  }
}
      ...
      if (line[0] == '*')
      {
        if (line.compare(0, 6, "* PONG") == 0)
        {
          if (sLogger.level().priority <= LDEBUG.priority) {
            sLogger << LDEBUG << "(Port:" << m_localPort << ")" << "Received a PONG for " << m_server << " on port " << m_port;
            dlib::timestamper stamper;
            int delta = (int) ((stamper.get_timestamp() - m_lastHeartbeat) / 1000ull);
            sLogger << LDEBUG << "(Port:" << m_localPort << ")" << "    Time since last heartbeat: " << delta << "ms";
          }
          if (!m_heartbeats)
...
void Connector::sendCommand(const string &aCommand)
{
  ...
  if (m_connected) {
    ...
    if (status <= 0)
    {
      sLogger << LWARN << "(Port:" << m_localPort << ")" << "sendCommand: Could not write command: '" << aCommand << "' - "
              << intToString(status);
    }
  }
}
...
void Connector::startHeartbeats(const string &aArg)
{
  ...
  if (aArg.length() > 7 && aArg[6] == ' ' && (pos = aArg.find_first_of("0123456789", 7)) != string::npos)  {
    ...
    {
      sLogger << LDEBUG << "(Port:" << m_localPort << ")" << "Received PONG, starting heartbeats every " << freq << "ms";
      m_heartbeats = true;
      m_heartbeatFrequency = freq;
    }
    else
    {
      sLogger << LERROR << "(Port:" << m_localPort << ")" << "startHeartbeats: Bad heartbeat frequency " << aArg << ", ignoring";
    }
  }
  else
  {
    sLogger << LERROR << "(Port:" << m_localPort << ")" << "startHeartbeats: Bad heartbeat command " << aArg << ", ignoring";
  }
}
...
void Connector::close()
{
  ...
  if (m_connected && m_connection.get() != NULL)
  {
    ...
    sLogger << LWARN << "(Port:" << m_localPort << ")" << "Waiting for connect method to exit and signal connection closed";
    if (m_connectActive)
