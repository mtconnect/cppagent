...
void AgentConfiguration::initialize(int aArgc, const char *aArgv[])
{
	...
	if (stat(m_configFile.c_str(), &buf) != 0)
	{
		if (!m_exePath.empty())
		{
		sLogger << LINFO << "Cannot find " << m_configFile << " in current directory, searching exe path: "
			<< m_exePath;
...
void AgentConfiguration::monitorThread()
{
	...
	if (check && (cfg_at_start.st_mtime != cfg.st_mtime ||
					  ...
					  bool aConversionRequired, bool aUpcaseValue)
{
	...
	for (block = blocks.begin(); block != blocks.end(); ++block)
	{
		...
		adp->setConversionRequired(get_bool_with_default(adapter, "ConversionRequired",
					   aConversionRequired));
...
void AgentConfiguration::loadStyle(dlib::config_reader::kernel_1a &aReader, const char *aDoc,
