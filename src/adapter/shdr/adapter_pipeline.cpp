...
namespace mtconnect
{
  ...
  namespace adapter
  {
    namespace shdr
    {
      std::unique_ptr<shdr::Handler> AdapterPipeline::makeHandler()
      {
        auto handler = make_unique<shdr::Handler>();
        ...
        handler->m_disconnected = [this](const std::string &id) {
          auto entity = make_shared<Entity>(
              "ConnectionStatus", Properties {{"VALUE", "DISCONNECTED"s}, {"source", id}});
          run(entity);
        };
        handler->m_processData = [this](const std::string &data, const std::string &source) {
          auto entity =
              make_shared<Entity>("Data", Properties {{"VALUE", data}, {"source", source}});
          run(entity);
        };
