...
using mqtt_client =
    mqtt_client_ptr<boost::asio::io_context &, std::string, std::uint16_t, mqtt::protocol_version>;
using mqtt_tls_client = mqtt_tls_client_ptr<boost::asio::io_context &, std::string, std::uint16_t,
    ...
    m_pipeline(std::move(pipeline))
{
  ...
  if (IsOptionSet(options, configuration::MqttTls))
    m_client =
        make_shared<MqttAdapterTlsClient>(m_ioContext, options, m_pipeline.get(), m_handler.get());
  else
    m_client =
        make_shared<MqttAdapterClient>(m_ioContext, options, m_pipeline.get(), m_handler.get());
