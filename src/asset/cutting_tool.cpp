...
FactoryPtr CuttingToolArchetype::getFactory()
{
  ...
  if (!tool)
  {
    ...
    static auto constraint = make_shared<Factory>(
        Requirements({Requirement("maximum", DOUBLE, false), Requirement("minimum", DOUBLE, false),
                      Requirement("nominal", DOUBLE, false), Requirement("VALUE", DOUBLE, false)}));

    static auto measurement = make_shared<Factory>(
        Requirements({Requirement("significantDigits", INTEGER, false), Requirement("units", false),
                      Requirement("nativeUnits", false), Requirement("code", false),
                      Requirement("maximum", DOUBLE, false), Requirement("minimum", DOUBLE, false),
                      Requirement("nominal", DOUBLE, false), Requirement("VALUE", DOUBLE, false)}));

    static auto measurements = make_shared<Factory>(
        Requirements({Requirement("Measurement", ENTITY, measurement, 1, Requirement::Infinite)}));
    ...
    static auto lifeCycle = make_shared<Factory>(Requirements(
        {Requirement("ReconditionCount", ENTITY, reconditionCount, false),
         Requirement("ToolLife", ENTITY, toolLife, 0, 3), Requirement("ProgramToolGroup", false),
...
FactoryPtr CuttingTool::getFactory()
{
  ...
  if (!tool)
  {
    static auto state = make_shared<Factory>(Requirements {
        {"VALUE",
         {"NEW", "AVAILABLE", "UNAVAILABLE", "ALLOCATED", "ALLOCATED", "MEASURED", "NOT_REGISTERED",
          "RECONDITIONED", "USED", "EXPIRED", "TAGGED_OUT", "BROKEN", "UNKNOWN"}}});
    ...
    lifeCycle->setOrder({"CutterStatus", "ReconditionCount", "ToolLife", "ProgramToolGroup",
                         "ProgramToolNumber", "Location", "ProcessSpindleSpeed", "ProcessFeedRate",
