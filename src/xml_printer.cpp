...
static inline void addAttribute(xmlTextWriterPtr writer, const char *key, const std::string &value)
{
  if (!value.empty())
    THROW_IF_XML2_ERROR(xmlTextWriterWriteAttribute(writer, BAD_CAST key, BAD_CAST value.c_str()));
}
...
void XmlPrinter::initXmlDoc(xmlTextWriterPtr writer, EDocumentType aType,
                            ...
                            const map<string, size_t> *count) const
{
  ...
  if (location.empty() && !mtcLocation.empty())
    ...
    location = xmlns + " http://schemas.mtconnect.org/schemas/" + rootName + "_" + m_schemaVersion +
               ".xsd";
