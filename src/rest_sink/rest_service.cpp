    ...
    m_logStreamData(GetOption<bool>(options, LogStreams).value_or(false))
{
  ...
  m_server->setErrorFunction([this](SessionPtr session, rest_sink::status st, const string &msg) {
...
void RestService::createAssetRoutings()
{
  ...
  if (m_server->arePutsAllowed())
  {
    auto putHandler = [&](SessionPtr session, RequestPtr request) -> bool {
      auto printer = printerForAccepts(request->m_accepts);
      respond(session, putAssetRequest(printer, request->m_body, request->parameter<string>("type"),
                                       request->parameter<string>("device"),
...
void RestService::createSampleRoutings()
{
  ...
  auto handler = [&](SessionPtr session, RequestPtr request) -> bool {
    ...
    if (interval)
    {
      streamSampleRequest(session, printerForAccepts(request->m_accepts), *interval,
                                        ...
                                        boost::system::error_code ec)
{
...

  {
    ...
    asyncResponse->m_session->writeChunk(
        content, asio::bind_executor(m_strand, boost::bind(&RestService::streamSampleWriteComplete,
                                                           this, asyncResponse)));
  }
}
...
void RestService::streamCurrentRequest(SessionPtr session, const Printer *printer,
                                       const int interval, const std::optional<std::string> &device,
                                       const std::optional<std::string> &path)
{
                            ...
                            const DevicePtr device, FilterSet &filter) const
{
  ...
  if (filter.empty())
  {
    ...
    throw RequestError(msg.c_str(), printError(printer, "INVALID_XPATH", msg), printer->mimeType(),
                       status::bad_request);
  }
}
