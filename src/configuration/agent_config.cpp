...
void AgentConfiguration::monitorThread()
{
  ...
  {
    LOG(warning) << "Monitor thread has detected change in configuration files, restarting agent.";
...
void AgentConfiguration::configureLogger(const ptree &config)
{
  ...
  b_logger::core::get()->add_global_attribute(b_logger::aux::default_attribute_names::thread_id(),
  ...
  else
  {
    ...
    m_sink->locked_backend()->set_file_collector(
        b_logger::sinks::file::make_collector(b_logger::keywords::target = log_directory,
  ...
  if (adapters)
  {
    for (const auto &block : *adapters)
    {
      ...
      if (protocol == "shdr")
      {
        ...
        auto adp = make_shared<adapter::shdr::ShdrAdapter>(m_context, host, port, adapterOptions,
                                                           pipeline);
      ...
      else if (protocol == "mqtt")
      {
        ...
        auto adp =
            make_shared<adapter::mqtt_adapter::MqttAdapter>(m_context, adapterOptions, pipeline);
        m_agent->addSource(adp, false);
        port = adp->getPort();
      }
      else
    ...
    for (const auto &block : *ns)
    {
      ...
      else
      {
        ...
        if (path && !location.empty())
        {
          ...
          if (!xns)
          {
            LOG(debug) << "Cannot register " << urn << " at " << location << " and path " << *path;
          }
        }
      }
    }
                                   ...
                                   rest_sink::FileCache *cache)
{
  ...
  if (files)
  {
    for (const auto &file : *files)
    {
      ...
      if (!location || !path)
      {
        LOG(error) << "Name space must have a Location (uri) or Directory and Path: " << file.first;
      }
      else
      {
