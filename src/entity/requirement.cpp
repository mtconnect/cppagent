...
bool Requirement::isMetBy(const Value &value, bool isList) const
{
  ...
  if ((m_type == ENTITY || m_type == ENTITY_LIST))
  {
    ...
    {
      throw PropertyError("Entity or list requirement " + m_name + " does not have correct type",
                          m_name);
    }
  }
  else
...
struct ValueConverter
{
  ...
  void operator()(const Timestamp &arg, int64_t &v) { v = chrono::system_clock::to_time_t(arg); }
