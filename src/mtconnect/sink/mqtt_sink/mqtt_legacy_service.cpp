...
#include "mqtt_legacy_service.hpp"
...
namespace mtconnect {
  namespace sink {
    namespace mqtt_sink {
      ...
      MqttLegacyService::MqttLegacyService(boost::asio::io_context &context,
                                           sink::SinkContractPtr &&contract,
                                           const ConfigOptions &options, const ptree &config)
        : Sink("MqttLegacyService", std::move(contract)), m_context(context), m_options(options)
      {
      ...
      std::shared_ptr<MqttClient> MqttLegacyService::getClient() { return m_client; }

      bool MqttLegacyService::publish(observation::ObservationPtr &observation)
      {
      ...
      bool MqttLegacyService::publish(device_model::DevicePtr device)
      {
      ...
      bool MqttLegacyService::publish(asset::AssetPtr asset)
      {
      ...
      void MqttLegacyService::registerFactory(SinkFactory &factory)
      {
        factory.registerFactory(
            ...
            [](const std::string &name, boost::asio::io_context &io, SinkContractPtr &&contract,
              ...
              auto sink =
                  std::make_shared<MqttLegacyService>(io, std::move(contract), options, block);
              return sink;
            });
      }
    }  // namespace mqtt_sink
