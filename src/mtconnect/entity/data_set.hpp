...
namespace mtconnect::entity {
  namespace data_set {
    ...
    struct Entry
    {
      ...
      /// @param removed `true` if the key has been removed
      Entry(std::string key, Entry &value, bool removed = false)
      : m_key(std::move(key)), m_value(std::move(value)), m_removed(removed)
      {}
      ...
      /// @param removed `true` if the key has been removed
      Entry(std::string key, T value, bool removed = false)
      : m_key(std::move(key)), m_value(std::move(value)), m_removed(removed)
      {}
    ...
    public:
      ...
      {
        return std::get<T>(find(ET(key))->m_value);
      }
  ...
  struct DataSetValueSameVisitor : public data_set::SameValueVisitor<DataSetValue>
  {
    using data_set::SameValueVisitor<DataSetValue>::SameValueVisitor;
