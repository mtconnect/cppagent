...
namespace mtconnect
{
  ...
  FactoryPtr Reference::getFactory()
  {
    ...
    if (!references)
    {
      auto abstractReference = make_shared<Factory>(
          Requirements {Requirement("idRef", false), Requirement("name", false)});

      auto componentRef = make_shared<Factory>(*abstractReference);

      auto dataItemRef = make_shared<Factory>(*abstractReference);

      references = make_shared<Factory>(Requirements {
          Requirement("ComponentRef", ENTITY, componentRef, 0, Requirement::Infinite),
          Requirement("DataItemRef", ENTITY, dataItemRef, 0, Requirement::Infinite)});
