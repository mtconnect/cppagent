...
Component::Component(const std::string &name, const entity::Properties &props) : Entity(name, props)
{
  m_id = get<string>("id");
...
FactoryPtr Component::getFactory()
{
  ...
  if (!factory)
  {
    ...
    factory->setOrder({"Description", "Configuration", "DataItems", "Compositions", "References"});
