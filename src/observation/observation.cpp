...
FactoryPtr Observation::getFactory()
{
  ...
  if (!factory)
  {
    ...
    // regex(".+TimeSeries$")
    factory->registerFactory([](const std::string &name) { return ends_with(name, "TimeSeries"); },
    ...
    factory->registerFactory([](const std::string &name) { return starts_with(name, "Samples:"); },
                             Sample::getFactory());
    factory->registerFactory([](const std::string &name) { return starts_with(name, "Events:"); },
                             Event::getFactory());
  }
  return factory;
}
...
FactoryPtr Event::getFactory()
{
  ...
  if (!factory)
  {
    ...
    factory->addRequirements(Requirements {{"VALUE", false}, {"resetTriggered", USTRING, false}});
  }
...
FactoryPtr Timeseries::getFactory()
{
  ...
  if (!factory)
  {
    ...
    factory->addRequirements(Requirements(
        {{"sampleCount", INTEGER, false}, {"VALUE", VECTOR, 0, entity::Requirement::Infinite}}));
  }
  return factory;
}
